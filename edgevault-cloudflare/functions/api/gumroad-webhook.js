...
//   resource_name === 'cancellation' -> downgrade to basic
//
// ---------------------------------------------------------------------
...
//
// ---------------------------------------------------------------------
// CANCELLATIONS NEED A SEPARATE ONE-TIME SETUP
// ---------------------------------------------------------------------
...
// Figures out 'basic' vs 'pro' from the ping's own variants field, e.g.
// { "Tier": "Pro" } or { "Tier": "Basic" }. Falls back to 'basic' if we
// can't find the word "pro" anywhere in the variant values.
function planFromVariants(variantsRaw) {
  ...
  return joined.includes('pro') ? 'pro' : 'basic';
}
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    if (resourceName === 'sale') {
      ...
      await patchDoc(env, `users/${owner.uid}`, { plan: 'basic' });
    }
